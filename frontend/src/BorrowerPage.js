    ...
    if (state.contract && borrowerAddress) {
      try {
        ...
        const formattedDetails = {
          requestedAmount: state.web3.utils.fromWei(details.requestedAmount.toString(), "ether"),
          repayAmount: state.web3.utils.fromWei(details.repayAmount.toString(), "ether"),
          interestRate: details.interestRate.toString(),
          state: statusMap[Number(details.state)],
          requestedDate: new Date(Number(details.requestedDate) * 1000).toLocaleString()
        };
